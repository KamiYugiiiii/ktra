...
void xoadautien(Tro &L){
    ...
    L = L->next;
    delete Q;
    // No need to display the list here, just update the head pointer.
}


void chenthuba(Tro &L, SinhVien X){
    Tro Q = L, P = new Node;
    P->infor = X;
    for(int i = 0; i < 1; i++){
        Q = Q->next;
...
int main() {
    ...
    cout<<"--------------------";
//	xoadautien(L);
	SinhVien X = taosv(1006,"Le Thi Doan",1998,7.6);
	cout<<"chen thu 3 \n";
	chenthuba(L,X);
	hienthidanhsach(L);
